...
extern int use_file_log;
extern char log_file[256];

void show_version(){
    logger(LOG_DEFAULT, "%s", PACKAGE_STRING);
    logger(LOG_DEFAULT, "%s", PACKAGE_COPYRIGHT);
    logger(LOG_DEFAULT, "Updated : %s", PACKAGE_LAST_UPDATE);
    logger(LOG_DEFAULT, "Developed by : %s", PACKAGE_BUGREPORT);
}
...
int add_config(char *p[], struct options* o){
    if(strcmp("config", p[0]) == 0){
        ...
        logger(LOG_DEBUGGING,"Config file : %s",o->config_file);
        return 0;
...
void init_options(struct options* o){
    log_level = 1;
    logger(LOG_DEBUGGING, "START INIT OPTIONS");

    o = malloc(sizeof(struct options));
    if(o == NULL){
         logger(LOG_DEBUGGING, "FAILED TO init");
    }
    CLEAR(*o);
    o->duplex = 0;
    o->layer_count = 0;
    ...
    logger(LOG_DEBUGGING, "END INIT OPTIONS");
}
...
int parse_argv(struct options* o, int argc, char* argv[]){
    ...
    if (argc == 2 && strncmp(argv[1], "--", 2)) {
        ...
        for (i = 1; i < argc; ++i)
        {
            ...
            p[0] = argv[i];
            if (strncmp(p[0], "--", 2)) {
                logger(LOG_DEFAULT, "Cannot parse option : %s", p[0]);
                return 1;
            }
            else {
                p[0] += 2;
...
int parse_config(struct options* o, char* p[]){
    if(strcmp(p[0], "duplex") == 0){
       ...
       if( strcmp(p[0],"none") != 0){
        ...
        logger(LOG_DEFAULT,"Unrecognized Options in Config, Exit Program. [%s]", p[0]);
        return 1;
    }
    return 0;
}
...
int read_config_file(struct options* o){
    ...
    if(fp == NULL){
        logger(LOG_DEFAULT,"Error Opening config file [%s]", o->config_file);
        return 1;
    }
...
int show_options(struct options* o){
    logger(LOG_DEBUGGING,"duplex :%s",o->duplex == 1 ? "on" : "off" );
    if(o->duplex == 1){
        logger(LOG_DEBUGGING,"mode   :%s",o->duplex == 1 ? "master" : "slave" );
        for (int i = 0;i < o->layer_count;i++){
            logger(LOG_DEBUGGING,"layer [%d]", i);
            logger(LOG_DEBUGGING,"   Gateway    :%s", o->l[i].gateway);
            logger(LOG_DEBUGGING,"   Virtual IP :%s", o->l[i].vip);
            logger(LOG_DEBUGGING,"   Netmask    :%s", o->l[i].netmask);
            logger(LOG_DEBUGGING,"   interface  :%s", o->l[i].interface);
            logger(LOG_DEBUGGING,"   Fail Count :%d", o->l[i].count);
            logger(LOG_DEBUGGING,"   duplex IP  :%s", o->l[i].dup);
        }
        logger(LOG_DEBUGGING,"HA     :%s",o->direct == 1 ? "on" : "off" );
        if(o->direct){
            logger(LOG_DEBUGGING,"HA IP  :%s",o->direct_ip);
            logger(LOG_DEBUGGING,"HA IF  :%s",o->direct_interface);
            logger(LOG_DEBUGGING,"HA PORT:%d",o->direct_port);
        }
        logger(LOG_DEBUGGING,"log    :%s",o->log_file);
        logger(LOG_DEBUGGING,"Syslog :%s",o->syslog ==1? "on" : "off" );
        logger(LOG_DEBUGGING,"Log lvl:%d",o->log_level);
        logger(LOG_DEBUGGING,"MG PORT:%d",o->management_port);
        return 0;
    }else{
        logger(LOG_DEFAULT,"Duplexer set off, Exit Program");
        return 1;
    }
}
