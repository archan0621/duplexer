...
int send_ping(int ping_sockfd, struct sockaddr_in *ping_addr, char *ping_dom, char *ping_ip, char *rev_host , int count)
{
    ...
    #endif
    {
        logger(LOG_DEFAULT,"Setting socket options to TTL failed!");
        return 1;
    } else {
    ...
    for(int i = 0; i< count; i++) {
        ...
        if ( sendto(ping_sockfd, &pckt, sizeof(pckt), 0,
           (struct sockaddr*) ping_addr,
            sizeof(*ping_addr)) <= 0){
            logger(LOG_DEBUGGING,"Packet Sending Failed!");
            flag=0;
        }
        ...
        if ( recvfrom(ping_sockfd, &pckt, sizeof(pckt), 0, (struct sockaddr*)&r_addr, &addr_len) <= 0 && msg_count>1) {
            logger(LOG_DEBUGGING,"Packet receive failed!");
        }else{
            ...
            if(flag){
                if(!(pckt.hdr.type ==69 && pckt.hdr.code==0)){
                    logger(LOG_DEBUGGING,"Error..Packet received with ICMP type %d code %d [%d]", pckt.hdr.type, pckt.hdr.code, i);
                }else{
                    logger(LOG_DEBUGGING,"%d bytes from %s (h: %s)(%s) msg_seq=%d ttl=%d rtt = %Lf ms.", PING_PKT_S, ping_dom, rev_host, ping_ip, msg_count, ttl_val, rtt_msec);
                    msg_received_count++;
                    break;
                }
            }
    ...
    logger(LOG_DEBUGGING,"[%s] %d packets sent, %d packets received, %f percent packet loss. Total time: %Lf ms.",ping_ip, msg_count, msg_received_count, ((msg_count - msg_received_count)/msg_count) * 100.0, total_msec);
    return 1;
}
...
int ping_main(char *address, int count) {
    ...
    if(ip_addr==NULL) {
        logger(LOG_DEBUGGING,"DNS lookup failed! Could not resolve hostname");
        return 1;
    }
 
    reverse_hostname = reverse_dns_lookup(ip_addr);
    logger(LOG_DEBUGGING, "Trying to connect to '%s' IP: %s", address, ip_addr);
    //logger(LOG_DEBUG, "Reverse Lookup domain: %s", reverse_hostname);

    sockfd = socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);
    if(sockfd<0){
        logger(LOG_DEBUGGING,"Socket file descriptor not received");
        return 1;
    }
