...
int install_vip(char* interface, char* ip){
	...
	close(fd);
    logger(LOG_DEFAULT, "Installed vip [%s] interface [%s]",ip ,interface );
...
int install_netmask(char* interface,char* netmask){
    ...
    logger(LOG_DEFAULT, "Installed netmask [%s] interface [%s]",netmask ,interface );
...
int check_vip(char* interface, char* ip){
	...
	if(strcmp(ip,inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr))==0){
		close(fd);
        logger(LOG_DEBUGGING, "Checked Installed IP Successfully");
		return 0;
	}else{
        logger(LOG_DEBUGGING, "Failed Check Ip : Installed IP is [%s]",inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
		close(fd);
		return 1;
	}
}
...
int down_vip(char* interface){
	...
	close(fd);
    logger(LOG_DEFAULT, "Interface %s is down", interface);
	return 0;
}