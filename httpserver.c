...
int getMessage(int fd, char* block) {
    ...
    if( (sstream = fdopen(fd, "r")) == NULL) {
        logger(LOG_DEBUGGING, "Error opening file descriptor in getMessage()\n");
        return 1;
    }
...
httpRequest parseRequest(char *msg, int* pilot){
    ...
    if( test != NULL ) {
        ...
        logger(LOG_DEBUGGING,"[ Master ]: My Plane");
        logger(LOG_DEBUGGING,"[ Slave  ]: Your Plane");
        *pilot = 1;
        ret.returncode = 200;
        memset(ret.filename , 0, 128);
        ...
        logger(LOG_DEBUGGING,"[ Master ]: Your Plane");
        logger(LOG_DEBUGGING,"[ Slave  ]: My Plane");
        *pilot = 2;
        ret.returncode = 200;
        memset(ret.filename , 0, 128);
...
int http_server(int port, int* pilot) {
    ...
    if ((list_s = socket(AF_INET, SOCK_STREAM, 0)) < 0 ) {
        logger(LOG_DEBUGGING, "Error creating listening socket.");
        return 1;
    }
    ...
    while(1){
        if (bind(list_s, (struct sockaddr *) &servaddr, sizeof(servaddr)) < 0 ) {
            logger(LOG_DEFAULT, "Error calling bind()");
            if(looper > 100){
                return 1;
            }
            sleep(1);
    ...
    while(1){
        ...
        {
            logger(LOG_DEFAULT, "Error Listening");
            if(looper > 100){
                return 1;
            }
            sleep(1);
    ...
    while(1)
    {
        ...
        if(conn_s == -1){
            logger(LOG_DEBUGGING,"Error accepting connection \n");
            continue;
        }
